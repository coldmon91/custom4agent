...
description: "Use when an approved implementation plan with mostly independent tasks is ready to execute, and the work should run in this session by dispatching one fresh subagent per task. Executes the plan task by task — implementer subagent, task review (spec compliance + code quality), bounded fix loop — then one whole-branch review at the end, with a compaction-proof ledger tracking progress. Trigger when the user says things like \"이 계획대로 구현 시작해줘\", \"플랜 실행해줘\", \"서브에이전트로 태스크별로 진행해줘\", \"execute this plan\", \"run the plan with subagents\", or right after the writing-plans skill produces an approved implementation plan. Do NOT trigger when no plan exists (run brainstorming then writing-plans first), when the tasks are tightly coupled and must be done as one edit, or for a single-file mechanical change."
---
...
- Run `git rev-parse --abbrev-ref HEAD`. Never start implementation on
  `main`/`master` without your human partner's explicit consent — create a
  feature branch, or a git worktree if the plan's work should not disturb the
  current checkout.
- Confirm the working tree is clean (`git status --porcelain`). Uncommitted
  work belonging to someone else makes every per-task diff unreadable.
- Every subagent works from this same directory; put its absolute path in
  each dispatch.
...
- **Task brief:** before dispatching an implementer, run this skill's
  ...
  (5) the report-file path and report contract; (6) the assigned difficulty
  Level and its one-line rationale. Exact values (numbers, magic strings,
  signatures, test cases) appear only in the brief. Never make a subagent
...
1. If it's a context problem, provide more context and re-dispatch at the same Level
2. If the task requires more reasoning, re-rate it one Level up and re-dispatch on the matching agent
...
dispatch a fresh implementer at the same Level carrying the brief path, the
...
- Report the branch name, the commit range, and a one-line summary of what
  the branch does.
- Report test status as it actually stands, including anything still failing.
- List the "Rulings I made" block, the deferred minors, and any parked
  findings the final review triaged as non-blocking.
- Ask before merging, pushing to a shared branch, or opening a PR — those
  are side effects outside this worktree.
