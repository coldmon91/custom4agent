...
| `sync/atomic` | Simple counters, flags | Prefer typed atomics: `atomic.Int64`, `atomic.Bool` |
...
| `sync.Once` | One-time initialization | `OnceFunc`, `OnceValue`, `OnceValues` |
| `sync.WaitGroup` | Waiting for simple goroutines | Prefer `wg.Go(func(){ ... })` for fire-and-wait tasks that do not panic and do not need error propagation, otherwise use `Add`/`Done`. For errors/cancellation/limits, use `errgroup` with context. |
...
For pipeline patterns (fan-out/fan-in, bounded workers, generator chains, iterators, `samber/ro`), see [Pipelines and Worker Pools](references/pipelines.md).
